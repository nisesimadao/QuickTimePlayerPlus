...
static void QTPCleanupRenderedMIDICache(void)
{
    ...
    for (NSURL *url in contents) {
        if ([url.pathExtension.lowercaseString isEqualToString:@"caf"] && [fileManager removeItemAtURL:url error:nil]) {
            removedCount++;
        }
    }
...
{
    QTPLog(@"Rendering MIDI for QuickTime window: %@", midiURL.path);
    ...
    if (![sequencer loadFromURL:midiURL options:AVMusicSequenceLoadSMF_ChannelsToTracks error:&error]) {
        if (outError) {
            *outError = error;
        }
        return nil;
